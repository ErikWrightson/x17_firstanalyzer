...
def printXPdf(X_histos, X_pdfName, lC, fo):
    ...
    latex.DrawLatex(0.5, 0.6, "3 Particle Candidate Document")
...
def printMPdf(M_histos, M_pdfName, lC, fo):
    ...
    Cut_desc = ["Cut 1 - 2 or More Clusters", 
                "Cut 2 - Timing",
                "Cut 3 - Fiducial",
                "Cut 4 - Kinematic (E_{exp})",
                "Cut 5 - Coplanarity",
                "Cut 6 - Elasticity",
                "Cut 7 - 1 GEM Match Each",
                "Cut 8 - 2 GEM Match Each",
                "Cut 9 - Vertex Z Cut for All"
                ]
    detail = ["Require that there are at least 2 clusters.",  
            "Require (#Deltat = t_{Max}-t_{Min}) < 16 ns.",
            "Require all clusters outside 1/2 of first",
            "Require each cluster to be within 5#sigma of",
    ...
    for i in range(0, len(xy_histos)):
        ...
        if fo == 2 or fo == 3 and i == 3: 
            print("HERE")
            latex.DrawLatex(0.5, 0.2, "SKIPPED THIS CUT")
        c.Update()
        c.Print(M_pdfName)
        c.Clear()
        if (fo == 2 or fo == 3) and i == 3: 
