...
namespace Physics_Utils{
     ...
     *
     * @return - a 4D Energy-Momentum Vector using the LorentzVector<PxPyPzE4D<Double_t>> class from ROOT
     */
    LorentzV make4D_EMomVector_Electron(Double_t E, DirVector p){
        Double_t P_mag = TMath::Sqrt(E*E - m_e*m_e);

        LorentzV v(P_mag*p.d_x, P_mag*p.d_y, P_mag*p.d_z, E);
     ...
     * or that it hit the PbWO4 or it hit under the absorber or halfway into the first open layer.
     * Only
    ...
    Int_t hycal_layer(Float_t x, Float_t y){
	    ...
	    if(TMath::Abs(x)>20.77*17.0 || TMath::Abs(y)>20.75*17.0 || (TMath::Abs(x)<(20.77*2.0+20.77/2.0) && TMath::Abs(y)<(20.75*2.0+20.75/2.0))){
    ...
    Double_t ee_ExpectedE(Double_t theta, Double_t EBeam){
        ...
        Double_t cosTheta = TMath::Cos(theta*deg2Rad);
