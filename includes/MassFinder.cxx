...
const TString MassFinder::MCUT_NAME[MCUT_NUM] = {"2 or More", "Timing", "Fiducial", "Coplanarity", "Elasticity", "1 GEM Match", "2 GEM Match", "Vertex Z"};
...
const TString MassFinder::XGGCUT_NAME[XGGCUT_NUM] = {"None", "3 or More", "Timing", "Fiducial", "Cluster E", "E Sum", "Coplanarity", "X_E", "No Match #gamma#gamma","1 GEM Match", "2 GEM Match", "Vertex Z"};
...
void MassFinder::save_histos(TString rootFile){
    ...
    for(Int_t i = 0; i < XGGCUT_NUM; i++){
        (*arr).Add(h_Xgg_HC_XY[i]);
        (*arr).Add(h_Xgg_E_theta[i]);
        (*arr).Add(h_Xgg_invM[i]);
        (*arr).Add(h_Xgg_Sum_pt[i]);
        (*arr).Add(h_Xgg_Sum_pxVpy[i]);
        (*arr).Add(h_Xgg_diffPhi[i]);
        (*arr).Add(h_Xgg_timing[i]);
        (*arr).Add(h_Xgg_sumE[i]);
        (*arr).Add(h_Xgg_minE[i]);
        (*arr).Add(h_Xgg_medE[i]);
        (*arr).Add(h_Xgg_maxE[i]);

        for(Int_t j = 0; j < 4; j++){
            (*arr).Add(h_Xgg_vZ[i][j]);
        }
    }
...
void MassFinder::search_events(){
    ...
    for(Int_t i = 0; i < entries; i++){
        ...
        for(Int_t j = 0; j < nClust; j++){
            ...
            //Fills the gamma-gamma histograms.
            h_Xgg_HC_XY[0]->Fill(cl_x[j], cl_y[j]);
            h_Xgg_E_theta[0]->Fill(theta[0], cl_E[j]);
            if(vZ[0]<90000){h_Xgg_vZ[0][0]->Fill(vZ[0]);} //Only fill histograms that have at least two points to rebuild the vertex to.
 ...
 * @note - a helper of the search_events function
 */
        ...
        if(layer[0] > 0 && layer[0] < 16 && layer[1] > 0 && layer[1] < 16 && layer[2] > 0 && layer[2] < 16){
            ...
            if(cl_E[j] > 70.0 && cl_E[j] < Ebeam*0.8 && cl_E[k] > 70.0 && cl_E[k] < Ebeam*0.8 && cl_E[m] > 70.0 && cl_E[m] < Ebeam*0.8){
                ...
                if(TMath::Abs(Ebeam-sum.E()) < sigma_E*sumRes){
                    ...
                    if(!(X_eli.at(0) < 0 && X_eli.at(1) < 0 && X_eli.at(2) < 0)){
                        cut++;
                        fillCutHistos_electron(cut, j, k, m);
 ...
 * @note - A helper for search_events method
 */
void MassFinder::fillIndInfo(Int_t i, Int_t j){
    ...
    /*Int_t GEMOrder[MAX_GEMS] = {1, 0, 3, 2};
    bool foundGEM = false;
    for(Int_t k = 0; k < MAX_GEMS; k++){
        if((match_flag[j] & (1<<GEMOrder[k])) && mgz[j][k]>0){
            theta[i] = findTheta(mgx[j][k], mgy[j][k], mgz[j][k]);
            phi[i] = findPhi(mgx[j][k], mgy[j][k]);
            dir[i] = makeDirVector(mgx[j][k], mgy[j][k], mgz[j][k]);
            foundGEM = true;
        }
        if((k == (MAX_GEMS-1)) && !foundGEM){
    ...
    particle_g[i] = make4D_EMomVector_Photon(cl_E[j], dir[i]);
...
void MassFinder::setup_Moller_histos(){
    ...
    for(Int_t i = 0; i < MCUT_NUM; i++){
        ...
        TString invM_Title = TString::Format("Moller Invariant Mass of Potential Particle 0.5 MeV per bin Run %d - Cut: %s;Mass [MeV/c^{2}]; Count", runNum, MCUT_NAME[i].Data());
...
void MassFinder::delete_histos(){
    ...
    for(Int_t k = 0; k < XGGCUT_NUM; k++){
        delete h_Xgg_HC_XY[k];
        delete h_Xgg_E_theta[k];
        delete h_Xgg_invM[k];
        delete h_Xgg_Sum_pt[k];
        delete h_Xgg_Sum_pxVpy[k];
        delete h_Xgg_diffPhi[k];
        delete h_Xgg_timing[k];
        delete h_Xgg_sumE[k];
        delete h_Xgg_minE[k];
        delete h_Xgg_medE[k];
        delete h_Xgg_maxE[k];
...
/**
 * Sets up the histograms for the X particle analysis expecting a [gamma - gamma] decay.
 */
void MassFinder::setup_Xgg_histos(){
    TString m[4] = {"All", "Min_E", "Med_E", "Max_E"};
    for(Int_t i = 0; i < XGGCUT_NUM; i++){

        TString xy_Name  = TString::Format("h_Xgg_HC_XY_cut%d", i);
        TString xy_Title = TString::Format("HyCal XY Position #gamma#gammae- Run %d - Cut: %s;x [mm]; y [mm]", runNum, XGGCUT_NAME[i].Data());
        h_Xgg_HC_XY[i] = new TH2F(xy_Name, xy_Title, HC_XBINS, HC_XMIN, HC_XMAX, HC_YBINS, HC_YMIN, HC_YMAX);

        TString th_Name  = TString::Format("h_Xgg_E_theta_cut%d", i);
        TString th_Title = TString::Format("Energy v. #theta #gamma#gammae- Run %d - Cut: %s;#theta [#circ]; Energy [MeV]", runNum, XGGCUT_NAME[i].Data());
        h_Xgg_E_theta[i] = new TH2F(th_Name, th_Title, TH_BINS, MIN_TH, MAX_TH, Ebeam+200.0, 0.0 ,Ebeam+200.0);

        TString invM_Name = TString::Format("h_Xgg_invM_cut%d",i);
        TString invM_Title = TString::Format("Invariant Mass of Potential Particle 0.5 MeV per bin #gamma#gammae- Run %d - Cut: %s;Mass [MeV/c^{2}]; Count", runNum, XGGCUT_NAME[i].Data());
        h_Xgg_invM[i] = new TH1F(invM_Name, invM_Title, INVM_BINS, MIN_INVM, MAX_INVM);

        TString sum_pt_Name = TString::Format("h_Xgg_Sum_pt_cut%d", i);
        TString sum_pt_Title = TString::Format("3 particle #Sigmap_{t} [#gamma#gammae-] Run %d - Cut: %s;p_{t} [MeV/c];Count", runNum, XGGCUT_NAME[i].Data());
        h_Xgg_Sum_pt[i] = new TH1F(sum_pt_Name, sum_pt_Title, PT_BINS, MIN_PT, MAX_PT);

        TString sum_pxVpy_Name = TString::Format("h_Xgg_pxVpy_cut%d", i);
        TString sum_pxVpy_Title = TString::Format("3 particle #Sigmap_{y} v. #Sigmap_{x} [#gamma#gammae-] Run %d - Cut: %s;p_{x} [MeV/c];p_{y} [MeV/c]", runNum, XGGCUT_NAME[i].Data());
        h_Xgg_Sum_pxVpy[i] = new TH2F(sum_pxVpy_Name, sum_pxVpy_Title, 2*PT_BINS, -1.0*MAX_PT, MAX_PT, 2*PT_BINS, -1.0*MAX_PT, MAX_PT);

        TString diffPhi_Name = TString::Format("h_Xgg_diffPhi_cut%d", i);
        TString diffPhi_Title = TString::Format("#Delta#phi of particle candidate and e' [#gamma#gammae-] Run %d - Cut: %s;#Delta#phi [#circ];Count/1#circ", runNum, XGGCUT_NAME[i].Data());
        h_Xgg_diffPhi[i] = new TH1F(diffPhi_Name, diffPhi_Title, PHI_BINS, MIN_PHI, MAX_PHI);

        TString timing_Name = TString::Format("h_Xgg_timing_cut%d", i);
        TString timing_Title = TString::Format("#Deltat (t_{cl_{max}} - t_{cl_{min}}) [#gamma#gammae-] Run %d - Cut: %s;#Deltat [ns]; Count/ns", runNum, XGGCUT_NAME[i].Data());
        h_Xgg_timing[i] = new TH1F(timing_Name, timing_Title, TIME_BINS, MIN_TIME, MAX_TIME);

        TString sum_E_Name = TString::Format("h_Xgg_sumE_cut%d", i);
        TString sum_E_Title = TString::Format("3 particle E_{Sum} [#gamma#gammae-] Run %d - Cut %s;E_{sum} [MeV]; Count/MeV", runNum, XGGCUT_NAME[i].Data());
        h_Xgg_sumE[i] = new TH1F(sum_E_Name, sum_E_Title, Ebeam+500.0, 0.0 ,Ebeam+500.0);

        TString min_E_Name = TString::Format("h_Xgg_minE_cut%d", i);
        TString min_E_Title = TString::Format("3 particle Minimum Cluster Energy E_{Min} [#gamma#gammae-] Run %d - Cut %s;E_{min} [MeV];Count/MeV", runNum, XGGCUT_NAME[i].Data());
        h_Xgg_minE[i] = new TH1F(min_E_Name, min_E_Title, Ebeam/2.0, 0.0 ,Ebeam/2.0);

        TString med_E_Name = TString::Format("h_Xgg_medE_cut%d", i);
        TString med_E_Title = TString::Format("3 particle Median Cluster Energy E_{Med} [#gamma#gammae-] Run %d - Cut %s;E_{med} [MeV];Count/MeV", runNum, XGGCUT_NAME[i].Data());
        h_Xgg_medE[i] = new TH1F(med_E_Name, med_E_Title, Ebeam, 0.0 ,Ebeam);

        TString max_E_Name = TString::Format("h_Xgg_maxE_cut%d", i);
        TString max_E_Title = TString::Format("3 particle Maximum Cluster Energy E_{Max} [#gamma#gammae-] Run %d - Cut %s;E_{max} [MeV];Count/MeV", runNum, XGGCUT_NAME[i].Data());
        h_Xgg_maxE[i] = new TH1F(max_E_Name, max_E_Title, Ebeam, 0.0 ,Ebeam);

        for(Int_t j = 0; j < 4; j++){

            TString vZ_Name = TString::Format("h_Xgg_vZ_%s_cut%d",m[j].Data(), i);
            TString vZ_Title = TString::Format("V_{z} %s [#gamma#gammae-] Run %d - Cut: %s", m[j].Data(), runNum, XGGCUT_NAME[i].Data());
            h_Xgg_vZ[i][j] = new TH1F(vZ_Name, vZ_Title, VZ_BINS, MIN_VZ, MAX_VZ);
        }
    }
    cout<<"Set up X particle [#gamma#gammae-] histograms." << endl;
}
 ...
 * @note - a helper of the search_events function
 */
void MassFinder::searchXEvent_photon(Int_t j, Int_t k, Int_t m){
    //Fill the summed energy and resolution for these 3 particles.
    sumRes = combined_EnergyRes(partEnergies);

    //Fill the information for the potential X candidates.
    Xgg[0] = particle_g[0] + particle_g[1]; //Particle 0 and 1
    Xgg_match[0] = oneGEM[0] || oneGEM[1];
    sumGG[0] = Xgg[0] + particle_e[2];

    Xgg[1] = particle_g[0] + particle_g[2]; //Particle 0 and 2
    Xgg_match[1] = oneGEM[0] || oneGEM[2];
    sumGG[1] = Xgg[1] + particle_e[1];

    Xgg[2] = particle_g[1] + particle_g[2]; //Particle 1 and 2
    Xgg_match[2] = oneGEM[1] || oneGEM[2];
    sumGG[2] = Xgg[1] + particle_e[2];

    Xgg_eli = {0, 1, 2};
    for(Int_t n = 0; n < 3; n++){
        Xgg_th[n] = findTheta(X[n].Px(), X[n].Py(), X[n].Pz());
        Xgg_phi[n] = findPhi(X[n].Px(), X[n].Py());
    }

    phi_dif_gg[0] = TMath::Abs(Xgg_phi[0]-phi[2]);
    phi_dif_gg[1] = TMath::Abs(Xgg_phi[1]-phi[1]);
    phi_dif_gg[2] = TMath::Abs(Xgg_phi[2]-phi[0]);

    //Cut 1 - Require at least 3 clusters to exist.
    cut_gg = 1;
    fillCutHistos_photon(cut_gg,j,k,m);
        ...
        if(layer[0] > 0 && layer[0] < 16 && layer[1] > 0 && layer[1] < 16 && layer[2] > 0 && layer[2] < 16){
            ...
            if(cl_E[j] > 70.0 && cl_E[j] < Ebeam*0.8 && cl_E[k] > 70.0 && cl_E[k] < Ebeam*0.8 && cl_E[m] > 70.0 && cl_E[m] < Ebeam*0.8){
                ...
                if(TMath::Abs(Ebeam-sum.E()) < sigma_E*sumRes){
                    ...
                    //Cut 6 - Require that the each invariant mass candidate and the e' are coplanar within 15 degrees.
                    for(Int_t d = 0; d < 3; d++){
                        if(TMath::Abs(180.0-phi_dif_gg[d]) > 15){Xgg_eli.at(d) = -1;}
                    }
                    //Only go deeper into the if statements if there are still elligible candidates.
                    if(!(Xgg_eli.at(0) < 0 && Xgg_eli.at(1) < 0 && Xgg_eli.at(0) < 0)){
                        cut_gg++;
                        fillCutHistos_photon(cut_gg, j, k, m);

                        //Cut 7 - Require that the each invariant mass candidate more than half the beam energy.
                        for(Int_t b = 0; b < 3; b++){
                            if(Xgg_eli.at(b) >= 0 && Xgg[b].E() < Ebeam*0.5){Xgg_eli.at(b) = -1;}
                        }
                        //Only go deeper into the if statements if there are still elligible candidates.
                        if(!(Xgg_eli.at(0) < 0 && Xgg_eli.at(1) < 0 && Xgg_eli.at(2) < 0)){
                            cut_gg++;
                            fillCutHistos_photon(cut_gg, j, k, m);

                            //Cut 8 - Require that both decay particles have no GEM matches.
                            for(Int_t a = 0; a < 3; a++){
                                if(Xgg_eli.at(a) >= 0 && Xgg_match[a]){Xgg_eli.at(a) = -1;}
                            }
                            //Only go deeper into the if statements if there are still elligible candidates.
                            if(!(Xgg_eli.at(0) < 0 && Xgg_eli.at(1) < 0 && Xgg_eli.at(2) < 0)){
                                cut_gg++;
                                ...
                                //Cut 9 - Require that the beam particle have at least 1 GEM match
                                for(Int_t s = 0; s < 3; s++){
                                    if(Xgg_eli.at(s) >= 0 && !oneGEM[2-s]){Xgg_eli.at(s) = -1;}
                                }
                                if(!(Xgg_eli.at(0) < 0 && Xgg_eli.at(1) < 0 && Xgg_eli.at(2) < 0)){
                                    cut_gg++;
                                    fillCutHistos_photon(cut_gg, j, k, m);
                                    //cout<<"z0: " << vZ[0] << " z1: " << vZ[1] << " z2: " << vZ[2] << endl;

                                    //Cut 10 - Require that the beam particle have at least a match from EACH GEM layer.
                                    for(Int_t f = 0; f < 3; f++){
                                        if(Xgg_eli.at(f) >= 0 && !twoGEM[2-f]){Xgg_eli.at(f) = -1;}
                                    }
                                    if(!(Xgg_eli.at(0) < 0 && Xgg_eli.at(1) < 0 && Xgg_eli.at(2) < 0)){
                                        cut_gg++;
                                        fillCutHistos_photon(cut_gg, j, k, m);

                                        //Cut 11 - Require that every particle have a track within 2 meters of the target.
                                        for(Int_t g = 0; g < 3; g++){
                                            if(Xgg_eli.at(g) >= 0 && TMath::Abs(vZ[2-g])>2000.0){Xgg_eli.at(g) = -1;}
                                        }
                                        if(!(Xgg_eli.at(0) < 0 && Xgg_eli.at(1) < 0 && Xgg_eli.at(2) < 0)){
                                            cut_gg++;
                                            fillCutHistos_photon(cut_gg, j, k, m);
...
/**
 * @brief Fills the histograms for the given cut and the indices of each particle assuming a gamma gamma decay.
 *
 * @param j - the index of the first particle to be included in the plots
 * @param k - the index of the second particle to be included in the plots
 * @param m - the index of the third particle to be included in the plots
 */
void MassFinder::fillCutHistos_photon(Int_t c, Int_t j, Int_t k, Int_t m){

    //Fill in the histograms that plot the individual cluster information.
    if(j == 0){ //Make sure to not accidentally double count particles.
        h_Xgg_HC_XY[c]->Fill(cl_x[j], cl_y[j]);
        h_Xgg_E_theta[c]->Fill(theta[0], cl_E[j]);

        if(vZ[0]<90000){
            h_Xgg_vZ[c][0]->Fill(vZ[0]);
        }
    }
    if(k == 1){ //Make sure to not accidentally double count particles.
        h_Xgg_HC_XY[c]->Fill(cl_x[k], cl_y[k]);
        h_Xgg_E_theta[c]->Fill(theta[1], cl_E[k]);

        if(vZ[1]<90000){
            h_Xgg_vZ[c][0]->Fill(vZ[1]);
        }
    }

    h_Xgg_HC_XY[c]->Fill(cl_x[m], cl_y[m]);
    h_Xgg_E_theta[c]->Fill(theta[2], cl_E[m]);
    h_Xgg_sumE[c]->Fill(sum.E());

    //Fill the vertex position for each particle and then separate by minimum, median, and maximum energy particle.
    //TString m[4] = {"All", "Min_{E}", "Med_{E}", "Max_{E}"};
    if(vZ[2]<90000){
        h_Xgg_vZ[c][0]->Fill(vZ[2]);
    }
    Int_t maxE_ind = distance(partEnergies.begin(), max_element(partEnergies.begin(), partEnergies.end()));
    if(vZ[maxE_ind]<90000){h_Xgg_vZ[c][3]->Fill(vZ[maxE_ind]);}

    Int_t minE_ind = distance(partEnergies.begin(), min_element(partEnergies.begin(), partEnergies.end()));
    if(vZ[minE_ind]<90000){h_Xgg_vZ[c][1]->Fill(vZ[minE_ind]);}
    
    Int_t medE_ind = 3 - (maxE_ind + minE_ind);
    if(vZ[medE_ind]<90000){h_Xgg_vZ[c][2]->Fill(vZ[medE_ind]);}

    h_Xgg_maxE[c]->Fill(partEnergies.at(maxE_ind));
    h_Xgg_minE[c]->Fill(partEnergies.at(minE_ind));
    h_Xgg_medE[c]->Fill(partEnergies.at(medE_ind));


    for(Int_t i = 0; i < 3; i++){
        if(Xgg_eli.at(i) < 0) continue;

        h_Xgg_invM[c]->Fill(Xgg[Xgg_eli.at(i)].M());
        h_Xgg_diffPhi[c]->Fill(phi_dif_gg[Xgg_eli.at(i)]);//TMath::Abs(X_phi[X_ind.at(i)]-phi[ep_ind]));
        h_Xgg_Sum_pt[c]->Fill(sumGG[Xgg_eli.at(i)].Pt());
        h_Xgg_Sum_pxVpy[c]->Fill(sumGG[Xgg_eli.at(i)].Px(), sumGG[Xgg_eli.at(i)].Py());
    }

   
    h_Xgg_timing[c]->Fill(dT);
